 ...
 * @param <V>
 */
public class SelectedPlanIDLogger<V extends DataType<V>> extends AgentLogger<Agent<V>> {
     ...
     */
    public SelectedPlanIDLogger(String filename, int totalNumAgents) {
    ...
    private String internalFetchingperRun(MeasurementLog log, int run) {
        ...
        selectedPlans.keySet().forEach(agentIdx -> {
            ...
            for (; true; i++) {
                ...
                selectedPlans.get(agentIdx).add(aggregate.getMin());
            }
