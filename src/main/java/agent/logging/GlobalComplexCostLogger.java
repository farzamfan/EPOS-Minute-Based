...
public class GlobalComplexCostLogger<V extends DataType<V>> extends AgentLogger<Agent<V>> {
	...
	public void log(MeasurementLog log, int epoch, Agent<V> agent) {
		...
		if(!(agent instanceof DynamicIncentiveIEPOSAgent)) {
			System.err.println("GlobalComplexCostLogger can only be used with DynamicIncentiveIEPOSAgent.");
			throw new ClassCastException();
		}
		
		DynamicIncentiveIEPOSAgent moagent = (DynamicIncentiveIEPOSAgent) agent;
...
//			parameters.put(OptimizationFactor.INCENTIVE_SIGNAL,moagent.getOptimization().incentiveSignal[agent.getIteration()]);
...
//          double[] complexCost = Configuration.planOptimizationFunction.apply(parameters);
//			double cost = (1-moagent.getUnfairnessWeight()-moagent.getLocalCostWeight()) * complexCost[0]+ (moagent.getLocalCostWeight()*complexCost[1]);

			double cost = (moagent.getComplexCosts(moagent.getIteration()).getValue(moagent.getComplexCostSelceted(moagent.getIteration())));
