 ...
 * @param <V>
 */
public class IncentiveVisualizerLogger<V extends DataType<V>> extends AgentLogger<TreeAgent<V>> {
	...
	public void log(MeasurementLog log, int epoch, TreeAgent<V> agent) {
		if(agent.getIteration() == agent.getNumIterations()-1) {
			double score = agent.getAggGainedIncentive()[agent.getIteration()];

			for(Plan<V> p : agent.getPossiblePlans()) {
				this.scoreSet.add(agent.getLocalCostFunction().calcCost(p,agent.getCostSignal()));
				if (agent.getPeer().getIndexNumber()==0){
                }
			}

			IncentiveVisualizerLogger.maxPlans = Math.max(IncentiveVisualizerLogger.maxPlans, agent.getPossiblePlans().size());
	...
	public void print(MeasurementLog log) {
		...
		for(int currentRun = 0; currentRun < Configuration.numSimulations; currentRun++) {
			...
			File file = new File(Configuration.outputDirectory + "/gained-incentive-graph-run-" + currentRun + ".png");
	...
	public Color getColor(IncentiveVisualizerLogger.TreeNode node) {
	    int val = (int) ((node.localCost+1)*(Configuration.planDim-1)/2);
		int mapping = (val * IncentiveVisualizerLogger.colors.length) / IncentiveVisualizerLogger.maxPlans;
		return IncentiveVisualizerLogger.colors[mapping];
	}
